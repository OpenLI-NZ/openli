...
static void destroy_voice_call(shared_voice_call_state_t *state,
    ...
    if (vc->sdpvalid) {
        HASH_DELETE(hh_sdp, state->cept_calls_by_sdpo, vc);
    }
...
static void _remove_voice_call(shared_voice_call_state_t *state,
    ...
    vc = kc->call;
    if (vc) {
        HASH_FIND(hh, vc->callids, kc->callid, strlen(kc->callid), found);
        if (found) {
...
void destroy_sip_call_state(shared_voice_call_state_t *state,
    ...
    HASH_ITER(hh, state->known_callids, kc, kctmp) {
...
static intercepted_voice_call_t *get_voice_call_by_callid(
        shared_voice_call_state_t *state, char *callid) {

    /* Read lock must be held prior to calling this function */
...
static intercepted_voice_call_t *get_voice_call_by_sessionid(
        shared_voice_call_state_t *state, char *sessionid) {
    ...
    HASH_FIND(hh_sessionid, state->cept_calls_by_sessionid, sessionid,
            strlen(sessionid), vc);
...
static intercepted_voice_call_t *get_voice_call_by_sdpo(
        shared_voice_call_state_t *state, sip_sdp_identifier_t *sdpkey) {
    ...
    HASH_FIND(hh_sdp, state->cept_calls_by_sdpo, sdpkey,
            sizeof(sip_sdp_identifier_t), vc);
...
static void add_new_callid_to_existing_voice_call(
    ...
    /* Write lock must be held prior to calling this function */
...
int find_existing_voice_call(
    ...
    if (lookup_callid) {
        if (lookup_sdpo == NULL && sdpkey) {
            // Same call ID, but a new SDP-O identifier?
            // Add it anyway -- we might be going through some weird proxying
            update_voice_call_identifiers(state, lookup_callid, sdpkey, NULL);

        }

        if (lookup_sessionid == NULL && sessionid) {
            // Call ID already exists, but a new session ID.
            // Again, unlikely but let's roll with it
            update_voice_call_identifiers(state, lookup_callid, NULL, sessionid);
        }
        ...
        if (lookup_sessionid) {
            add_new_callid_to_existing_voice_call(state,
...
void remove_target_call_reference(voipintercept_t *vint, uint32_t cin) {
    ...
    HASH_ITER(hh, vint->target_cin_map, tgtref, reftmp) {
        ...
        if (tgtmap) {
            ...
            if (tgtmap->callid) free(tgtmap->callid);
            free(tgtmap);
        }
    }
